...
- **It said "hard-reset the LFO on every clock pulse"** — that contradicts the actual use case (one *slow* cycle spanning the whole sequence: A on the downbeat, U mid-sequence, back to A). The right model is **continuous phase-lock to Transport at a chosen division** (incl. multi-bar), not per-pulse reset. Also dropped Gemini's redundant extra Division switch + separate OFFSET knob + S&H (deferred) in favour of reusing existing knobs.
...
**Deferred:** Sample & Hold (stepped/snapping vowels) — needs a real sample-hold node; own follow-up.
...
**Critical review of the directive (Gemini "Phase 54"):** most of it was already shipped or wrong for the as-built code, and was rejected/redirected:
- **Phase number collision** — Moog Phase 54 already shipped (Rack densification). This is Phase 63.
- **CASE_CONFIG + CABLE_PATCHES persistence — already done** (Phase 60f `moog-rack-v2`). Not re-implemented.
- **Cable schema** — Gemini's 4-field `(SrcModule, SrcPort, DstModule, DstPort)` is redundant: cables store globally-unique jack ids `{from, to}` where the id already IS `moduleId-port`. Kept the 2-field form.
- **`RackManager` / `cases` array / "total DOM re-render"** — none exist; the rack is `dynModules` state + the store helpers + a wrapping expansion row, and the architecture is deliberately no-remount. Not introduced.
- **The one genuine gap = knob/switch VALUE persistence** (`MODULE_SETTINGS`) — every module's positions were local `useState` lost on reload. This was the real work. (Dylan chose the full-vision scope.)

**As built:**
- **Store schema** — `moog-rack-v2` gains `settings: { [instanceId]: {values} }`, keyed by canonical instance id (the jack prefix — `vco1`, `vcf`, `seq2`, `env1`, `io`…). `readRackStore` defaults it to `{}`; `SCHEMA` unchanged so old records load fine.
...
2. **Unpowered slave leak:** the hard-sync worklet runs continuously (`process()` returns true forever), so `powerOff` force-gates `syncOut` to 0. But `setVcoNSyncEnabled`/`setVcoSyncEnabledById` opened `syncOut=1` based only on `enabled`, ignoring power — so enabling sync (or the 63c-#1 mount restore) while unpowered leaked the free-running slave straight to the I/O. **Fix:** `syncOut` opens only when `enabled && isPoweredRef.current` (all 5 statics + the dynamic variant); the enabled-ref is still recorded so `powerOn` restores the crossfade. **Verified:** enabling sync while unpowered now keeps `syncOut=0` (was 1 → leak), power-on opens it to 1; reload with `syncOn=true` restores correctly; zero errors.

Diagnosis note for future: a hard-synced saw's spectral PEAK sits at the slave formant, not the master fundamental — don't mistake that for "free-running." Confirm sync by A/B-ing with vs without the master patched (the slave-freq energy vanishes when synced).
...
**Two approaches tried and DISCARDED (do not retry):**
- *Motion mode* (hide knob layers via `opacity:0` during scroll): worked (smooth) but **Dylan rejected the visual** — knobs/glow vanishing during a pan. Also carried two landmines if ever revived: (a) NO `transition: opacity` on `.knobGroup` — animating opacity over 130 will-change layers is ~30× worse; (b) the hide/show attribute must never flicker — each toggle create/destroys 130 layers, jamming the thread longer than a frame, so a fixed `setTimeout(clear)` races its own jam into a runaway (needs a self-rescheduling idle poll).
- *Animation freeze* (pause LED/meter/canvas rAF writes during a pan via an `isCameraBusy` flag / `viewportActivity.js`): keeps everything visible but only ~25 % off a real pan (the knob-layer composites dominate, not the animation) — not enough alone, and it froze the LED/display animation during scroll (a behavior change). Superseded by module promotion, which is both smoother AND keeps animation live. `viewportActivity.js` deleted.
- Also measured-worse at real pan: removing knob `will-change` entirely (raster-on-pan, 3781 ms — catastrophic); flatten-to-one-cabinet-layer.

**Why module (not knob, not cabinet) is the sweet spot:** knob layers = 130 per-frame composites (too many); no layers = whole-cabinet raster-on-pan (catastrophic); one cabinet layer = over-rasterized huge texture; ~30 module layers = few composites AND bounded raster-on-pan (each module cached after first exposure). GPU memory drops too (30–60 module layers < 130 knob layers). Black-flash risk not increased (modules are small/bounded, and knobs were already permanently promoted).

### [2026-07-12] Moog Phase 61b — Smooth powered scrolling: SIZE-GATE the culling manager
...
**Root cause (Playwright matrix, headed for real GPU raster):** it is **Phase 61's own culling manager**. Toggling a module's `content-visibility` invalidates the layer tree → one `Layerize` pass per toggle; during a scroll the visible band moves every frame, so the manager churns toggles → a `Layerize` every frame → main thread saturates, and because the camera's wheel handler is non-passive (main-thread), wheel input lags too. On the default rack (1799 px / 29 modules) the manager still culls 21/29 at idle and re-promotes them on every scroll frame — **pure cost**: the rack idles fine at 8 ms without culling. Isolation on the default rack, powered: lights-on+manager **p95 83 ms**; lights-out **p95 9 ms, 0 frames >25 ms**; lights-on with the manager force-disabled (all modules pinned rendered) **p95 16 ms**. The last two ≈ each other and crush the first → the manager is the differentiator.

**Two hypotheses tested and REJECTED before landing the fix:**
- *Writer-pause* (pause LED/meter/canvas/chase writes during gestures via a `viewportActivity` busy flag): implemented across 5 files, then **reverted** — it did not move the needle (the writers were never the scroll cost; the manager's toggling was) and added cross-file complexity. Dylan also disliked it.
- *Remove knob `will-change: transform`* (the ~278 permanent compositor layers): made large-rack scroll **worse** (p95 58 → 117 ms). `will-change` correctly keeps each knob on a GPU layer so a pan is a cheap texture move; removing it forces repaint-on-pan. **Kept as-is.**

**Fix (one gate, ~6 lines):** the manager engages only when `cabinet.offsetHeight > CULL_MIN_NATH` (2500 layout px — between the default rack's 1799 and a genuinely large rack's 3415; offsetHeight is layout height, zoom/viewport-independent, so the gate is stable). Below the threshold the effect clears all `content-visibility` and returns → the default rack (and any modest custom rack) renders every module and scrolls like lights-out. Large custom racks still cull (their 30 fps idle problem is real; their scroll is layer-bound and laggy either way, so keeping the idle win is the right trade). `moduleVisRef` is left null when gated off — the camera's `apply()` already `?.`-guards it.

**Verified (Playwright, powered):** DEFAULT rack scroll **p95 83 → 9.3 ms (locked 60 fps+)**, all 29 modules `content-visibility: unset` (gate off), idle unchanged 8.5 ms. LARGE rack unchanged — gate on (18 modules culled), idle 17.6 ms, cables 18/18. Zero console/page errors. The Phase 61 LED/meter write dedupe (harmless, real idle win) stays.
...
**Files modified:** none (measurement only; Playwright harness lives in the session scratchpad)

Gemini's directive (react-window module virtualization, `will-change` on knobs/LEDs, canvas cable rendering with culling, scroll-event throttling) was rejected on evidence: the rack is a single transform-driven camera surface, not a DOM scroller — off-screen content isn't painted during pans, the cable overlay lives inside the transformed cabinet and pans for free, knob/LED `will-change` already exists, and no scroll-driven UI updates exist to throttle. Unmounting off-screen modules would break `jackRefs`-based cable endpoint resolution (committed cables read jack rects at render time). The phase number also collided: Phase 52 shipped 2026-06 as the period-correct System 55 visual pass. Measured reality (near-cap 31-instance rack): unpowered scroll is flawless (p50 8.3 ms @ 120 Hz); the actual defect is the **powered** rack idling at ~30 fps from per-frame ~31 ms compositor layerization passes — shipped as **Moog Phase 61** (entry above). Key isolation results retained here: JS was idle (<5% incl. GC) while `Layerize` consumed 2.6 s of every 3 s; suppressing all app rAF loops restored 9.7 ms means; neutralizing all 278 knob `will-change` layers, freezing opacity writes, freezing canvas draws, and killing CSS animations each did NOT help individually (any one surviving writer class suffices). **Non-causes (do not revisit):** cable SVG count, scroll-event handling, static knob/LED `will-change` layers, power-lamp flicker animation, feTurbulence plates (pre-rasterized background-image data URIs, not live filters). Side observation: the Realtime AudioWorklet thread runs ~52% busy on this rack (10 hard-sync VCOs + 3 QNT worklets) — a separate budget with no main-thread impact.
