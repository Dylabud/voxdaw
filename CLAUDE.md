...
**Moog ↔ Workstation bridge (Phase 152):** Root holds `moogApiRef` (the Moog registers `{ getBusNode, resetSequencers, isPowered }` via `onBusReady` on mount) and `moogRecordingActiveRef` (a shared flag). Root hands the Workstation stable getters `getMoogBusNode`/`resetMoogSequencers`/`isMoogPowered` and a `setMoogRecordingActive` setter; it hands the Moog `recordingActiveRef`. **Moog + Workstation share the global `Tone.Transport`** (the Moog sequencer runs on `Tone.Loop`s on it), so the Workstation's Moog-record flow (`handleMoogRecord`: idle → 1-bar count-in → record) seeks Transport to the selected region's bar, `resetSequencers()`, then starts Transport so the Moog sequence + the backing timeline play together while the recorder taps `moogBus`. `KeyboardModule`'s QWERTY guard lifts while `recordingActiveRef.current` is true so computer keys play the Moog live into the take (its **MIDI path never had the guard** — a real MIDI keyboard always reaches the mounted Moog).
...
`bounceProject({ tracks, regions, notes, bpm, globalAutomations, groups, audioBuffers = new Map(), tailSec = null, capSec = Infinity })` → `{ buffer, firstOnsetSec }`. Render duration = rightmost region end + tail, hard-capped at `capSec` (callers pass `totalMeasures * 4 * 60/bpm` — the growable right stop). **Recorded Moog audio (`audioBuffers`, regionId → native `AudioBuffer`, from `audioBuffersByRegionId` — session-only, NOT persisted):** a region with `hasAudio` **and** a matching buffer renders that buffer raw-to-destination via a `Tone.Player` at `secondsAtMeasure(startMeasure)` (matching the live player), **gated on track audibility** (mute/solo), and its **transcription notes are skipped** (no synth Part) — the transcription is edit-only. A `hasAudio` region with no buffer falls through to the note path (transcription survives a project load; the buffer doesn't). `contentEndSec` is extended so a recording ringing past its drawn region isn't truncated, and `firstOnsetSec` folds in the recording's start tick. The **same `hasAudio` skip lives in the live region reconciler** (`useWorkstationAudio`, `computePartKey` includes `hasAudio`) so the recorded audio and its transcription never layer live either. `hasAudio` is a reliable "live recording exists this session" flag *because it is not serialized* in `projectIO`. Known limitation: recorded audio bypasses the track volume/pan/FX chain in both live and export (matches the live `.toDestination()` player) — routing it through the chain is a follow-up. `tailSec: null` means auto: `estimateFxTailSec` bounds the ring-out from audible tracks' non-bypassed delay (−60 dB through the feedback loop) and reverb (`1 + roomSize*9` s heuristic), clamped to [2, 30] s. `firstOnsetSec` = earliest note onset across **audible** tracks (analytic, from `buildRegionEvents` ticks), or `null` when nothing audible is scheduled. Both export paths (`handleExportAudio`, `exportTrack`) then call `trimExportBuffer(buffer, { startSec: firstOnsetSec })` (`audioExport.js`): drops everything before the first onset and trailing silence after the last sample above −80 dBFS peak (+0.25 s pad); returns `null` → "nothing to export", no file. **Start-trim is analytic, never a front signal-scan** — a threshold scan would eat `fadeInFloor: 0` fade-ins and slow attacks. `exportWAV`/`exportMP3` stay whole-buffer and untrimmed (RecordModal depends on them as-is).
