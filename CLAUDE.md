...
The app shell is a thin state-router — **no `react-router-dom`**. `Root.js` owns `page` (`'home' | 'voxtool' | 'workstation' | 'moogmodular' | 'aigen'`), `isDarkMode`, and `pendingProject` state, applies `data-theme="light"` on a top-level wrapper, and threads `onNavigateHome` + theme props to each page. Pages mount on first visit (`visited` Set) and stay mounted `display:none` for audio continuity. `App.js` is the VoxTool page; Root renders `WorkstationShell` directly (the old under-construction `Workstation.jsx` wrapper was removed). Adding a new page = adding a branch in `Root.js`, not a routing library.

### AI Instrument Generator (`src/components/AIGen/`, `src/hooks/useAIInstrument.js`)
Standalone page (`'aigen'`): prompt bar → Claude returns a **synth patch JSON** (never audio) → local graph `PolySynth → Filter → [makeFxGraph…] → Volume → Destination` behind the workstation's `KeyboardPanel` + a `RotaryKnob` chassis (master/ADSR/voice/filter/octave + one dynamic group per patch effect from `EFFECT_DEFS` metadata). Invariants:
- **`sanitizePatch` (patchSchema.js) is the trust boundary** — the model returns **prompted JSON** (not structured outputs — see network-layer bullet), so sanitizePatch is the SOLE validator: it drops unknown effect types/params and clamps every model number to registry min/max before it reaches a Tone param (this is what upholds the fxChain load-bearing ranges: delay.time ≤ 1.0, feedback ≤ 0.9, roomSize ≤ 0.95), always returning a complete playable patch from arbitrary input. Never feed a raw model patch to `applyPatch`.
- **Single writer per node** (hook setters own filter/volume/synth/fx params); Filter+Volume persistent, synth+FX rebuilt per patch (`patchSynth.makePatchSynth` — a peer to synthFactory, do not merge them). FX use bare `makeFxGraph`, not `makeFx` (no bypass UI).
- **Network layer is raw `fetch`** (`claudeService.js`, header `anthropic-dangerous-direct-browser-access`) — `@anthropic-ai/sdk` does NOT bundle under stock CRA (its credential chain imports `node:fs`; webpack UnhandledSchemeError). Don't reinstall it without adding a webpack-override layer. System prompt is a static module constant (cache-friendly) ending in a `## Response format` block; the reply is **prompted JSON** parsed by `parsePatchJson` (first `{` … last `}`) then sanitized — **NOT `output_config` structured outputs** (the 16-effect discriminated-union schema exceeds Anthropic's compiled-grammar size limit; `PATCH_SCHEMA` was removed). No `temperature`/`thinking` params (400 on newer models).
- API key lives ONLY in `localStorage['voxdaw.anthropicKey']` (in-app ⚙ settings); model in `localStorage['voxdaw.aigenModel']`. Never in the repo, `.env`, `.voxdaw`, or undo.
- The QWERTY window listener carries the InstrumentPanel guards — `offsetParent === null` (hidden-page silence) and `inTextInput()` (the prompt bar is an input) are both load-bearing.
- `applyPatch`/`generatePatch` have a `mode: 'sample'` **stub seam** for a future text-to-audio → `Tone.Sampler` backend; patch mode is the implemented path.
- **MIDI input is a PEER hook** (`useMidiInput.js`, Web MIDI API) — do NOT confuse with `useMidi.js`, which is MIDI *output* over a WebSocket bridge. Note-on/off route to the same `handleNoteOn(note, vel)/handleNoteOff` as the keys; gated on `rootRef.offsetParent === null`.
- **Custom instruments = a patch baked into a composite node** (`customInstrumentSynth.makeCustomInstrumentNode`: `makePatchSynth → Filter → [makeFxGraph…] → Volume`). It is NOT a `Tone.PolySynth` but the engine treats it as one — trigger/`set`/`releaseAll`/`dispose`/**`toSeconds`** delegate to the inner PolySynth, `.connect` to the output Volume, `activeVoices` is exposed (the duck-typers `synthIsRinging` and `applyEnvelope` need no change). **`toSeconds` is load-bearing** — `Tone.Part` callbacks call it; omitting it crashes playback ("toSeconds is not a function"). **Glide IS supported** via `makeMonoGlideVoice` (a mono composite reproducing filter/FX; exposes the `detune`/`volume` the glide scheduler writes) — only **drum kits** disable the glide tool. Registry `customInstruments.js` (id `custom:<uuid>`): library (localStorage `voxdaw.customInstruments`, dropdown "custom" optgroup) ∪ project-registered (`registerInstruments`). `makeSynth`/`defaultEnvelopeFor`/`makeGlideVoice` resolve custom ids; missing id → default synth.
- **`.voxdaw` embeds used custom-instrument defs** (`serializeProject`'s `customInstruments` via `customInstrumentsForIds`; `applyProjectData` → `registerInstruments` BEFORE the audio reconciler builds) so imports on another machine reconstruct them. Additive — **SCHEMA_VERSION intentionally NOT bumped**. A registered instrument isn't permanent until saved to the library: `applyProjectData` prompts on load for not-yet-`isInLibrary` instruments, and the track ContextMenu's **Add Instrument** (`addToLibrary`) persists one on demand. Performance keyboard octave floor is `MIN_OCT = -2` (both AIGen + workstation InstrumentPanel); pressed-key visual is `.keyDown` ONLY (never `:active` — it sticks to the mousedown element through a drag).
