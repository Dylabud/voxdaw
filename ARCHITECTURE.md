...
A patch: `{ name, voice: { engine: 'simple'|'fm'|'am', oscillator, harmonicity?, modulationIndex?, modulationOscillator? }, envelope: {a,d,s,r}, filter: null|{type,frequency,q}, effects: [{type, params}], volume }`. The model returns the patch as **prompted JSON** (the system prompt asks for a bare JSON object; the reply is `JSON.parse`d then sanitized) — **not** `output_config` structured outputs: the `effects` field is a discriminated `anyOf` over all 16 effect types, and its compiled strict grammar exceeds Anthropic's grammar-size limit ("compiled grammar is too large"). **`sanitizePatch()` is therefore the sole validator** — every patch (model-generated or loaded from a `.voxdaw`) passes through it before any number reaches a Tone param. It clamps to registry min/max — which IS the load-bearing fxChain set (delay.time ≤ 1.0, feedback ≤ 0.9, roomSize ≤ 0.95, compressor.threshold ≤ −0.1) — drops unknown effect types/params, caps the chain at `MAX_EFFECTS` (4), validates selects/toggles, and always returns a complete playable patch from arbitrary input. Unit-tested (`patchSchema.test.js`).

### Audio layer (`useAIInstrument.js` — peer hook, rule 4)
Graph: `PolySynth → Tone.Filter → [makeFxGraph₁…ₙ] → Tone.Volume → Destination`. Filter + Volume are persistent (the filter always exists — a `filter: null` patch just sets it transparent at lowpass 18 kHz / Q 0.7071); the synth (`patchSynth.makePatchSynth` — a peer to `synthFactory`, mirroring `voiceSpecFor`'s option shapes, `maxPolyphony` 16) and FX graphs are rebuilt per patch by `applyPatch` (regeneration cuts ringing tails — same accepted limitation as workstation structural FX rebuilds). FX reuse `fxChain.makeFxGraph` directly (bare `{in,out,apply,dispose}` — no bypass UI, so no `makeFx` wrapper). Single-writer setters: `setVolume` (0.05s ramp) / `setEnvelope` (reuses `synthFactory.applyEnvelope`) / `setVoiceParam` / `setFilter` / `setEffectParam(i,…)` (`graph.apply(p, 0.02)` — fxChain's own live constant). `noteOn` gates the attack on `Tone.start()` (first-gesture AudioContext resume). `applyPatch` opens with a clearly-marked `mode: 'sample'` stub seam for a future text-to-audio → `Tone.Sampler` backend.

### Network layer (`claudeService.js`)
Raw `fetch` to `POST https://api.anthropic.com/v1/messages` with the `anthropic-dangerous-direct-browser-access: true` CORS opt-in header — **not** `@anthropic-ai/sdk`: the SDK's credential chain dynamically imports `node:fs`/`node:path`, which stock CRA webpack 5 cannot bundle (UnhandledSchemeError) without a config-override layer. The system prompt is a module-level constant (static bytes = prompt-cache-friendly; the user's description is the only user-message content) whose per-effect section is generated from `EFFECT_DEFS` (ranges/defaults) plus a hand-written one-line sonic character per type, and which ends with a `## Response format` block asking for a bare JSON object. The reply is parsed with `parsePatchJson` (slices between the first `{` and last `}`, tolerating stray prose / ```json fences) then `sanitizePatch`d — **no `output_config` structured outputs** (the 16-effect discriminated-union schema exceeds Anthropic's compiled-grammar size limit; `PATCH_SCHEMA` was removed). No `temperature`/`thinking` params (rejected by newer models). `stop_reason 'refusal'` → friendly message; errors map by HTTP status (`401` → open key settings). Models: `claude-opus-4-8` (default) / `claude-sonnet-4-6` / `claude-haiku-4-5`, selectable in the prompt bar (~2–3¢ / 1.5¢ / 0.5¢ per generation).

### UI layer (`AIInstrumentGenerator.jsx` + module CSS)
Header (home / settings / theme) → prompt bar (input + model select + generate) → patch title row → chassis → keybed. The chassis reuses `RotaryKnob` + `InstrumentPanel.module.css` classes: master vol (dB), ADSR, voice group (harmonicity, + mod idx when fm — hidden for simple), filter (type select, log cutoff, res), octave stepper, and **one dynamic group per patch effect** iterating `EFFECT_DEFS[type].params` (numeric → knob, toggle → pill, select → mini-select) — zero per-effect UI code. Knob handlers write audio imperatively through the hook setters AND `setPatch` for the visual (Zero-Re-render: audio never waits on a render); double-click restores the value the AI chose (`generatedPatchRef`). The keybed is the workstation's `KeyboardPanel` (purely presentational) with the same chromatic QWERTY map; the window key listener carries all four InstrumentPanel guards — incl. `rootRef.offsetParent === null` (pages stay mounted `display:none`; without it the hidden synth plays from other pages) and `inTextInput()` (doubly load-bearing: the prompt bar is an input).
