...
## 12. Three-Theme System (dark / slate / light)

### Mechanism (Phase 159)
`Root.js` owns a `theme` string state (`'dark' | 'slate' | 'light'`, replacing the original `isDarkMode` boolean), lazily initialized from `localStorage['voxdaw.theme']` (validated against the known set, falls back to `'dark'`; a `useEffect([theme])` writes it back — persistence, unlike the original mechanism, which reset to dark every reload). The wrapper div sets `data-theme={theme === 'dark' ? undefined : theme}`: dark stays the bare `:root` default (byte-identical to the original hardcoded palette), `slate` and `light` are explicit attribute values. `src/utils/theme.js` is the single source of the theme vocabulary: `THEME_ORDER = ['dark','slate','light']`, `THEME_GLYPHS = {dark:'●', slate:'◑', light:'○'}` (fill = darkness), `THEME_LABELS`, and `nextTheme(t)` for the cycle.

**Slate** is a lighter dark mode (grey `#2c2f34`…`#3c3f45` surfaces instead of near-black charcoal) added for legibility in bright rooms — it keeps the mint accent and mirrors dark's surface *ordering* (base < panel < elevated < surface; light mode inverts this relationship, so slate deliberately does not copy light's structure, only its brightness).

### CSS Variable Layers (`src/index.css`)
Three blocks — bare `:root` (dark), `[data-theme="slate"]`, `[data-theme="light"]` — each defining the same ~29 variables:
| Category | Variables |
|----------|-----------|
...
| Piano-roll shading | `--pr-shade-natural`, `--pr-shade-accidental` |
| Loop | `--loop-tint` |
...
Every page's `◑`-style button (Controls sidebar / VoxTool, Workstation transport bar, AIGen header, HomePage header) now **cycles** dark → slate → light → dark on click, glyph = `THEME_GLYPHS[theme]`, title names the *next* theme. `theme` + `onThemeToggle` props flow from `Root.js` down through `App`/`WorkstationShell`/`AIInstrumentGenerator`/`HomePage` (and further into `Controls`/`RegionEditor`). The **HomePage additionally has a ⚙ gear** next to its toggle, opening a small dropdown (clones the `ProjectCard` kebab-menu pattern: capture-phase outside-mousedown close) listing all three themes with the active one accent-marked — direct selection via a `Root`-supplied `onThemeSelect={setTheme}` prop, not a cycle.

### Canvas-redraw invariant
The Workstation timeline-ruler grid and the RegionEditor piano-roll grid repaint via `getComputedStyle` reads of the live CSS vars, gated behind a `useLayoutEffect` whose dependency array **must carry the `theme` string, never a derived boolean** — a boolean collapses dark and slate to the same "is dark" value and would silently skip the repaint on a dark↔slate switch.
...
Soundtrap-style project dashboard: header (wordmark + nav buttons for VoxTool / Workstation / Moog Modular + the cycling theme toggle + the ⚙ theme-settings gear, §12), a centered single-column `PROJECTS` list of wide low-height rows (max-width 780px; class name `.card` kept from the original grid card so the `.card:hover .kebabBtn` hover-reveal rule still applies), and a footer alpha/privacy note. A slim dashed `+ NEW PROJECT` row sits above the list (`onOpenProject({ projectId: null, data: null })`); each saved project renders a `ProjectCard.jsx` row — name + `{trackCount} tracks · {bpm} bpm` + short updated-at on the left, an **audio preview** (▶ + waveform, see below) in the middle, a hover kebab menu on the right (rename via inline input → `renameProject`, duplicate, download `.voxdaw`, delete with confirm). Card click → `getProject` → `deserializeProject` → `onOpenProject`. An `[ import .voxdaw ]` button validates via `deserializeProject`, stores a new record, then opens it. **Grid refresh:** HomePage stays mounted, so Root passes `active={page === 'home'}` and a `useEffect` re-lists (projects AND preview metas) on return, plus after every mutating action. IndexedDB-open failure renders an inline "storage unavailable" note instead of crashing.

**Audio previews (Phase 158, Soundtrap-style):** any full-mix export (wav or mp3, never a per-track stem) from the Workstation persists the bounced mix as a WAV preview keyed to the saved project. A row's ▶ + `PreviewWave.jsx` (a tiny SVG traced from stored peak buckets — no audio decode on the dashboard) is playable **only when the preview still matches the currently saved project version**; otherwise it's greyed with a "preview outdated — export to update" tooltip. Freshness is a **content hash** (`hashProjectData`, `projectIO.js` — FNV-1a over the deterministic `serializeProject` JSON, deliberately **excluding `name`** so a rename never stales a preview), stamped on the project record at save time and on the preview record at export time; they're compared, never timestamps (two independent actions can never share an exact instant). Playback is a single shared lazy `HTMLAudioElement` (one preview at a time; stops on navigating away since the page stays mounted `display:none`).

### Browser project store (`src/utils/projectStore.js`)
Native IndexedDB (db `voxdaw`, **v3**) behind a small promise wrapper — no dependency. Two stores:
- **`projects`** (keyPath `id`): `{ id: crypto.randomUUID(), name, createdAt, updatedAt, bpm, trackCount, dataHash, data }` — `data` is the full `serializeProject()` output (source of truth; bpm/trackCount/dataHash are denormalized for card display and freshness comparison). API: `listProjects()` (metadata only incl. `dataHash`, newest first), `getProject`, `saveProject` (upsert; stamps `updatedAt`, preserves existing `createdAt`), `deleteProject` (also clears the project's preview keys in the same transaction), `renameProject` (patches both the record name and `data.name`; the hash is left untouched by design — see above).
- **`previews`** (out-of-line string keys, added v2→v3): two records per project, `wav:<id>` → raw WAV `ArrayBuffer` and `meta:<id>` → `{ projectId, peaks, durationSec, dataHash, exportedAt }`, **meta always written last** (customSampleStore's completeness-marker pattern — a half-written preview from a cancelled/failed export is invisible to `listPreviewMetas()`). API: `savePreview`, `getPreviewWav`, `listPreviewMetas`, `deletePreview`, `copyPreview` (project duplication copies the preview + hash — the copy's data differs from the source only by `name`, which the hash ignores, so it's instantly fresh).

A failed `openDB()` clears the cached promise so the next call can retry.

The Workstation's `[ save ]` writes here (upsert under `currentProjectId` state; first save mints the id; stamps `dataHash`) — producing a `.voxdaw` file moved to the export menu (`.voxdaw` item alongside mp3/wav, filename from the project name). Both `handleSaveProject` and the audio-export preview stamp read from one shared `buildProjectPayload()` builder so the two hash sites can never serialize differently. `[ load ]` (file input) still opens raw files; it resets `currentProjectId` to `null` so the first save after a file open creates a new store record. The project `name` travels inside the `.voxdaw` file as an **additive field** (`SCHEMA_VERSION` intentionally not bumped) and is editable via a click-to-edit `NAME` block in the transport bar (the BPM-editor pattern).
...
**Single-writer:** `recomputeTempo` in the hook is the ONLY writer of `Transport.bpm` (the old shell writers in handlePlayPause/handleBpmCommit/applyProjectData are gone; the tempo-lane drag preview `previewTempo` in the shell is the one transient exception). It rebuilds `tempoMapRef` and re-schedules the curve from the current tick position, and runs FIRST inside `recomputeFades`. **Re-anchoring MUST use `cancelAndHoldAtTime`, never `cancelScheduledValues`** (Phase 146): `Transport.ticks` is the INTEGRAL of the bpm curve, and a plain cancel mid-ramp deletes the in-flight ramp's end event — rewriting the already-*played* curve shape, so the musical position jumps (Parts re-fire notes at the edit spot) and TickParam's per-event tick bookkeeping desyncs, driving `getTimeOfTick`'s quadratic into NaN/negative times → the `TickSource.getTicksAtTime` "reading 'time' of undefined" crash (which also leaks a corrupt temp state event, so it keeps crashing until reload). `cancelAndHoldAtTime` truncates the ramp at the hold point instead, preserving the played integral exactly; `previewTempo` uses the same hold (it also replaces the pre-ramp `setValueAtTime`) and is **gated to a non-running transport** — while playing, a point drag must not pull the live tempo to the held value; the playing curve only updates on the mouseup commit (recomputeTempo). Ordinary Params (fades, volume/pan/fx automation) don't integrate ticks — their plain-cancel pattern stays. **Second invariant (Phase 146b): never two differing-value bpm events at one timestamp.** Stacked lane points used to schedule the step as `ramp(b1, T)` + `set(b2, T)` — but `TickParam.getTimeOfTick` resolves the ramp's endpoint via `getValueAtTime(after.time)`, and `Timeline.get` is last-wins, so it solved the quadratic against the POST-step value; a downward step drives the discriminant negative → `sqrt(NaN)` → the same crash (repro: a point ramping into two stacked points). All bpm scheduling therefore goes through **`tempoScheduleOps(map, nowM, baseTime)`** (`tempoMath.js`, unit-tested; shared by `recomputeTempo` and the offline bounce): strictly increasing event times — ramp to the FIRST stacked value at its true time (follows the drawn line), then `set` each subsequent value `TEMPO_STEP_EPS = 1e-3` s later; the leading re-anchor set sits at `baseTime + eps` so it never shares `cancelAndHoldAtTime`'s internal same-time ramp+set pair. — so fades and `recomputeAutomation` (which consume the map) and all 7 existing re-anchor call sites cover tempo for free; a delta-keyed tempo-sync effect (1f) covers lane/bpm edits. **Everything that converts measures↔seconds goes through the map:** fade envelope durations resolve at FIRE time inside the `Transport.schedule` callback (this killed the old "BPM change mid-region desyncs fades" caveat; `computeFadeKey` has no bpm component), track-automation ramp anchors, note durations in live+offline Part callbacks (event `"<ticks>i"` spans → map seconds), bounce duration/capSec/firstOnsetSec, and both 30 Hz stepped drivers read `transport.ticks/(PPQ·4)`. Offline bounce schedules the same anchor ramps on the offline transport (starts at 0 ⇒ map seconds = context seconds). **Unmount hazard handled:** `Transport.cancel(0)` does NOT clear bpm-signal automation and the Transport is shared with VoxTool's transport-clocked arp — cleanup cancels the bpm schedule and restores the flat base value. Live readout: when the lane has points, the rAF loop writes `∿<bpm>` to a span beside the base BPM (zero re-render).
...
**Audio:** per-group chain `memberMutes → groupIn(Gain) → groupVolume → groupPan(inert, pinned 0) → [group FX] → groupMute → Destination`. **Group pan is a member OFFSET, not a bus pan** (Phase 146 — a bus `Panner` after the member pans collapses individual stereo positions): effective member pan = `clampPan(track.pan + group.pan)`, applied on the MEMBER panners by pan sync #3b (which stores the applied *effective* value and never mutates `track.pan` — a fully-panned member pins at the wall and returns to its own value when the group un-pans). The bus `Panner` stays only as the FX-splice head effect 1b expects per family. A group PAN automation lane uses the same offset semantics: `resolveAutomationTarget` (and the bounce's `panEntries` override) resolves it to one ramp entry per member panner with `map = clampPan(memberBase + offset)`; a member with its own pan lane keeps it (skipped from the group entries); grouped members carry `groupId:pan` in effect 1e's chanKey so a member knob turn re-bases the scheduled ramps; release-restore clears the MEMBER panApp entries so #3b re-owns them. Effect #0g (before #1) reconciles bus nodes; effect #1a re-parents CONNECTED track mutes on membership change (disconnected ones pick the target up from `connectTrack`'s `outputForTrack`); FX effects 1b/1c/1d run a `syncFamily` pass per channel family (`e<n>` ids span both, so per-effect delta maps need no re-keying); automation resolution/release-restore/live-preview UNION the track+group node maps (`t<n>`/`g<n>` disjoint); effect #3e syncs group volume (only — pan has no bus writer) under the automation-takes-over contract. **Audibility rule (group mute + solo):** `anySoloed` spans tracks AND groups; track audible = `!t.isMuted && !group.isMuted && (!anySoloed || t.isSolo || group.isSolo)`; group bus audible = `!g.isMuted && (!anySoloed || g.isSolo || any member soloed)` — `audibleByTrackIdRef` stores the EFFECTIVE value, so the fire-time note skip and `connectTrack` gate are group-blind. **Pruning:** buses get the same ramp→delayed-disconnect as tracks, and the 1 Hz sweeper prunes a bus once no member is connected past max(member windows) + `estimateGroupTailSec` (FX-only bound; shared `fxTailSec` extraction); `connectTrack` revives the bus before its own early-return. **Bounce:** offline group chains mirror live behind one shared `computeAudibility`; `estimateFxTailSec` SUMS member tail + bus tail (serial rings add); per-track stem export neutralizes group mute/solo (a muted parent must not silence the stem; the stem still prints through the group FX). **Persistence:** additive `groups` + `track.groupId`, dangling-id/empty-group repair, `nextGroupId`, e<n>/a<n> scans include group arrays; groups are inside undo snapshots (Delete Group is undoable). `deleteTrack` GCs a group whose last member dies.

### Import project as group (Phase 158, `projectImport.js` / `WorkstationShell.jsx`)
A `[ import group ]` transport-bar dropdown (saved-dashboard picker via `listProjects()`, or a `.voxdaw` file) merges an ENTIRE other project into the running session as ONE new track group — the multi-track generalization of `duplicateTrack`'s id-remap pattern. The foreign project is parsed through the ordinary `deserializeProject` (repair pass + sanitizers reused as-is) — **never `applyProjectData`**, which replaces the whole session instead of merging into it. The pure `remapProjectAsGroup(clean, counters)` (`projectImport.js`, unit-tested) mints fresh `t/r/note_/e/a` ids from the CURRENT session's live counters (never inside a setState updater — the StrictMode eager-mint rule), selects notes by regionId ownership with trackId reconciled (the `duplicateTrack` lesson), remaps fx-target automations through per-track effect-id maps (dropping + counting orphans), and wraps every imported track in one fresh `g<n>` group. Because groups can't nest, **the imported project's OWN groups are dissolved** (their bus volume/pan/FX/automations discarded — counted in the toast) and its **tempo `globalAutomations` are dropped** (the host project's tempo wins; note times are musical so content still lines up). Regions keep their `startMeasure` (bar 1 stays bar 1); imported tracks append at the end of the array (a contiguous run, satisfying the group-header-per-run invariant above); `totalMeasures` grows to cover the imported content. Custom-instrument ids are content-addressed and never remapped — `registerInstruments` runs on the embedded defs before the state commit, with the same not-yet-`isInLibrary` confirm prompt `applyProjectData` uses. The whole merge is applied as ONE batch of `setState` calls so React 18 batching turns it into a **single undo entry**.
...
- **Dark and slate themes:** `--pr-shade-natural = rgba(255,255,255,…)` (brighten naturals), `--pr-shade-accidental = transparent` — slate reuses dark's direction (it's still a dark mode), just a slightly stronger alpha since the base is lighter.
...
**Tool modes (Phases 147–148):** the notes tab has a `toolMode` segmented toggle next to the snap select (`'editor' | 'velocity' | 'glide'`, ref-mirrored as `toolModeRef` for the window drag handlers). *Editor* is the original move/resize/draw behavior. *Velocity* freezes note geometry entirely: selection (click / shift-click / marquee) is unchanged, empty-grid clicks only deselect, and a vertical drag edits velocity — screen-space `dy`, 1 px = 1 unit, one shared integer delta across the selected cluster, clamped **1–120** per note (default 100). Per the Zero-Re-render Rule the drag writes only DOM: each note's `data-vel-bar` child (left-anchored black bottom bar, width `velocity/120` as a *percentage* so a single write is valid for the base note and its region-clipped loop ghosts) plus a singleton `.velReadout` above the anchor note; mouseup commits once via `onCommitNoteEdits([{ id, velocity }])` (one undo entry via the burst coalescer). Velocity reaches audio as `(ev.velocity ?? 100)/120` on the Tone trigger calls — carried on `buildRegionEvents` events, consumed by live `makePartCallback` and both `audioBounce` callbacks, and **included in `computePartKey`** (edits must rebuild Parts). Persisted additively via `sanitizeVelocity` in `projectIO.js` (SCHEMA_VERSION unchanged); carried through the note clipboard; `transcribeAudio` emits the same 1–120 scale.

**Glide (Phase 148):** *Glide* mode (disabled on drum tracks) renders an SVG overlay over the grid (AutomationLane pattern — `pointer-events: none`, handles opt back in; notes frozen + dimmed). Per note: a left anchor (`startOffset` 0–1, where the pitch ramp starts), a right anchor snapped to semitone rows (`endPitch`, with glissando audition + pitch readout during the drag), and a midpoint tension handle bending the line into `g(u) = u + tension·u(1−u)` — the drawn quadratic bezier (control `cy = y0+(y1−y0)(0.5+tension/2)`) is mathematically identical to the audio curve. Data: additive `note.glide = { startOffset, endPitch, tension, connected }`, pruned to `undefined` when inert (`normalizeGlide`); connections are **resolved, never stored** — the target is the unique same-region note whose start tick (`tickOf`) equals the host's end tick at `endPitch` (survives id repair / paste / moves; claw delete keeps the bend, the freed note re-attacks). `compileGlideChains` (pure, `glideMath.js`, inside the shared `buildRegionEvents`) merges connected runs into ONE chain event with per-member `segments` — consumed members never emit events (true legato: single attack, one release at chain end) — behind a one-scan `hasGlide` bail so glide-free regions take the original path. `planGlideSegments` converts segments to schedulable ops (seconds offsets from chain start; tempo-map-exact via `measureAtSeconds`; ≥64-sample cents curves — smooth, never staircase), consumed by two shared live+offline schedulers: **synth family** → per-region pools of bare mono voices (`makeGlideVoice` via `voiceSpecFor`, timbre-identical to the PolySynth voices) with `detune` curve scheduling and relative-dB `volume` crossfades (cap 8/region, 4 reduced-quality, oldest-stolen, lazy, silenceAll cancels scheduled curves); **sampler family** → the captured `_activeSources` buffer source with `playbackRate = r0·2^(cents/1200)` curves, release at chain end, and gain crossfades clamped inside its fadeIn/fadeOut windows (buffer-length truncation accepted). Right-click in glide mode opens the shared ContextMenu as `targetType: 'glideNote'` (per-open `glideFlags`, greyed unless exactly one next note starts at the end tick; multi-select maps chords top-pitch→top-pitch, unmapped lowest notes skipped).
...
## 18. AI Instrument Generator (`src/components/AIGen/`, `src/hooks/useAIInstrument.js`)

Standalone page (`page='aigen'`, `[ AI Instruments ]` on the HomePage nav): describe an instrument in a prompt bar → a Claude model returns a **synth patch JSON** (not audio) → the patch drives a locally-owned Tone.js graph behind a playable 2-octave keyboard with a full knob chassis. Three decoupled layers plus an isolated network module.

### Data layer (`patchSchema.js` — pure, no Tone/React)
A patch is a **stack of 1–5 layers**: `{ name, volume (master out), layers: [{ voice: { engine: 'simple'|'fm'|'am', oscillator, harmonicity?, modulationIndex?, modulationOscillator?, spread?/count? (fat osc), portamento? }, envelope: {a,d,s,r}, filter: null|{type,frequency,q}, effects: [{type, params}], volume, octave, semitone }, …] }`. **Envelope decay reaches 20 s** (`ENV_META.decay.max = 20`, log knob) so the model decides PER LAYER whether a held note fades out (sustain 0 + decay = fade time — pianos/bells/plucks) or holds forever (sustain > 0 — organs/pads); the system prompt spells out both idioms and a stack may mix them. `samePatchStructure(a, b)` (pure, tested) reports whether two patches share layer count / voice engines / oscillator types / effect-type sequences — the guard for the undo fast path below. **`layers` is an additive, backward-compatible field** — a legacy flat patch (voice/envelope/… at the top level) normalizes to a single layer (its `volume` becomes master; the layer sits at unity 0 dB), so old library/`.voxdaw` patches load byte-identically and SCHEMA_VERSION is untouched. Per-layer pitch offset `layerDetuneCents(layer) = octave*1200 + semitone*100` cents is the single source both the poly and glide paths read. The model returns the patch as **prompted JSON** (the system prompt asks for a bare JSON object; the reply is `JSON.parse`d then sanitized) — **not** `output_config` structured outputs: the `effects` field is a discriminated `anyOf` over all 16 effect types, and its compiled strict grammar exceeds Anthropic's grammar-size limit ("compiled grammar is too large"). **`sanitizePatch()` is therefore the sole validator** — every patch (model-generated or loaded from a `.voxdaw`) passes through it before any number reaches a Tone param. It clamps to registry min/max — which IS the load-bearing fxChain set (delay.time ≤ 1.0, feedback ≤ 0.9, roomSize ≤ 0.95, compressor.threshold ≤ −0.1) — drops unknown effect types/params, caps each layer's chain at `MAX_EFFECTS` (4) and the stack at `MAX_LAYERS` (5), validates selects/toggles, and always returns a complete playable patch from arbitrary input. Unit-tested (`patchSchema.test.js`).

### Audio layer (`useAIInstrument.js` — peer hook, rule 4)
Graph: **N per-layer subgraphs** `makePatchSynth(layer) → Tone.Filter → [makeFxGraph₁…ₙ] → layerVol → muteGain`, all summed into a persistent master `Tone.Volume → Destination`. Only the master Volume is persistent; every layer's synth/filter/FX/level/mute is (re)built per patch by `applyPatch` via the shared `customInstrumentSynth.buildLayerChain` (the filter always exists — a `filter: null` layer sets it transparent at lowpass 18 kHz / Q 0.7071; regeneration cuts ringing tails — same accepted limitation as workstation structural FX rebuilds). Each layer's synth is `patchSynth.makePatchSynth(layer, { maxPolyphony })` — a peer to `synthFactory`, mirroring `voiceSpecFor`'s option shapes, folding in fat-osc `spread`/`count` + `portamento` and setting the layer's `detune` from `layerDetuneCents`; the generator passes a generous **64** cap so long stacked chords don't drop/warn (the workstation keeps 16 + quality-tier governance). FX reuse `fxChain.makeFxGraph` directly (bare `{in,out,apply,dispose}` — no bypass UI, so no `makeFx` wrapper). Layer-indexed single-writer setters: `setVolume` (master, 0.05s ramp) / `setLayerVolume(i,…)` / `setLayerMute(i,muted)` (dedicated per-layer gain — **transient**, never persisted) / `setLayerPitch(i, layer)` (writes `layerDetuneCents` = octave+semitone) / `setEnvelope(i,…)` (reuses `synthFactory.applyEnvelope`) / `setVoiceParam(i, setObj)` (nested Tone `.set` partial) / `setFilter(i,…)` / `setEffectParam(i, idx,…)` (`graph.apply(p, 0.02)` — fxChain's live constant). `noteOn` gates the attack on `Tone.start()` (first-gesture AudioContext resume) then fans to every layer. `applyPatch` opens with a clearly-marked `mode: 'sample'` stub seam for a future text-to-audio → `Tone.Sampler` backend. **`applyPatchParams(patch)`** is the param-only sibling: it re-drives every chassis-editable parameter (master/layer volumes, envelope, detune, voice params, filter, FX params) against the LIVE graph with zero disposal — the undo/redo fast path when `samePatchStructure` holds, so ⌘Z no longer cuts ringing tails; it must be kept in sync with `patchVoiceSpec`'s fields (a missed param = undo silently not restoring audio).

### Network layer (`claudeService.js`)
Raw `fetch` to `POST https://api.anthropic.com/v1/messages` with the `anthropic-dangerous-direct-browser-access: true` CORS opt-in header — **not** `@anthropic-ai/sdk`: the SDK's credential chain dynamically imports `node:fs`/`node:path`, which stock CRA webpack 5 cannot bundle (UnhandledSchemeError) without a config-override layer. The system prompt is a module-level constant (static bytes = prompt-cache-friendly) whose per-effect section is generated from `EFFECT_DEFS` (ranges/defaults) plus a hand-written one-line sonic character per type, and which ends with a `## Response format` block asking for a bare JSON object; the **user** message = the description **plus** a per-request layer cap line (`"Use at most N layers"`, N = `maxLayersForTier(tokenTier)`) so the cap rides the uncached turn and the system prompt stays byte-identical. The reply is parsed with `parsePatchJson` (slices between the first `{` and last `}`, tolerating stray prose / ```json fences) then `sanitizePatch`d — **no `output_config` structured outputs** (the 16-effect discriminated-union schema exceeds Anthropic's compiled-grammar size limit; `PATCH_SCHEMA` was removed). No `temperature`/`thinking` params (rejected by newer models). `stop_reason 'refusal'` → friendly message; `max_tokens` → "switch tokens to high/xtra or use fewer layers"; errors map by HTTP status (`401` → open key settings). Models: `claude-opus-4-8` (default) / **`claude-fable-5`** / `claude-sonnet-4-6` / `claude-haiku-4-5`, selectable in the prompt bar. **Token tiers** (prompt-bar select, persisted to `localStorage['voxdaw.aigenTokens']`): `medium`/`high`/`xtra` = `max_tokens` **2048/8192/16384** — a ceiling, not a cost (billed on tokens actually generated). **`xtra` unlocks up to 5 layers** (`maxLayersForTier`; others cap at 3) — needed because a 5-layer JSON exceeds the smaller budgets.

### UI layer (`AIInstrumentGenerator.jsx` + module CSS)
Header (home / settings / theme) → prompt bar (input + token-tier select + model select + generate + **undo/redo** + save) → patch title row → **Master Mix panel** → active-layer **detail chassis** → keybed. The **Master Mix panel** holds a MASTER section (master volume knob + keyboard-octave stepper — the global playback octave) and a row of **channel strips, one per layer, that double as the layer selector**: a clickable `L#` header sets the active layer (active strip highlighted) + a `×` remove, then a volume knob, an **octave** stepper, a **semitone** stepper, and a **mute** toggle; a `+ layer` button (capped at `maxLayersForTier(tokenTier)`) sits at the end. The **detail chassis** (below, reusing `RotaryKnob` + `InstrumentPanel.module.css`) edits the ACTIVE layer only: ADSR, voice group (portamento always; harmonicity + mod idx for fm/am; fat-osc voices/spread), filter (type select, log cutoff, res), and **one dynamic group per patch effect** iterating `EFFECT_DEFS[type].params` (numeric → knob, toggle → pill, select → mini-select) — zero per-effect UI code. Knob handlers write audio imperatively through the layer-indexed hook setters AND `setPatch` for the visual (Zero-Re-render: audio never waits on a render); double-click restores the value the AI chose (`genPatchRef`, mirrored per layer). **Render-path discipline (Phase 157):** `RotaryKnob` and `KeyboardPanel` are `React.memo`'d, and every hot-path knob handler is identity-stable — built once via `useMemo`/`useCallback` reading live state through inline-synced `patchRef`/`activeLayerRef` (env/voice handler maps, per-index layer-volume array, per-`(fxIndex,key)` cached FX handlers with call-time meta lookup) — so a knob drag re-renders ONLY the dragged knob instead of the whole page; `buildMelodicMap` is memoized once and shared by the QWERTY listener and the keyboard's `hotkeys` prop. New hot-path controls MUST keep handler identity stable or the memoization silently dies. **Undo/redo** is a local history of whole-patch snapshots with a burst-coalescer (one entry per knob drag — pre-edit patch captured once, committed ~350 ms after the last change; structural add/remove-layer commit immediately) driven by buttons + a dedicated ⌘/Ctrl+Z window listener (guarded by `inTextInput()`); a new generation clears the stacks. Mute + keyboard octave are excluded (transient). The keybed is the workstation's `KeyboardPanel` (purely presentational) with the same chromatic QWERTY map; the window key listener carries all four InstrumentPanel guards — incl. `rootRef.offsetParent === null` (pages stay mounted `display:none`; without it the hidden synth plays from other pages) and `inTextInput()` (doubly load-bearing: the prompt bar is an input).

### Key storage
The Anthropic API key is entered in-app (⚙ settings), persisted **only** to `localStorage['voxdaw.anthropicKey']` — never in the repo, `.env`, or the bundle. Model choice persists to `localStorage['voxdaw.aigenModel']`. Neither enters `.voxdaw` files or undo.

### MIDI input, save, and rename (generator page)
- **MIDI input** (`useMidiInput.js`, a peer hook — Web MIDI API, **distinct** from the output-over-WebSocket `useMidi.js`): enumerates `requestMIDIAccess().inputs`, a prompt-bar select binds one device, its `onmidimessage` maps note-on/off (`0x90` vel>0 / `0x80`|`0x90`-vel0) → `Tone.Frequency(n,'midi').toNote()` → the same `handleNoteOn(note, vel)/handleNoteOff` the on-screen keys use (velocity threads into `useAIInstrument.noteOn` → `triggerAttack(note, undefined, vel)`). Message handler is gated on `rootRef.offsetParent === null` like the QWERTY listener.
- **Editable patch name**: a gray raised box (`.nameBox`) — double-click or right-click ("edit name") → inline input (Enter/Escape/blur, workstation rename idiom); commit updates `patch.name` + `genPatchRef`.
- **Save ▾ dropdown**: "to workstation" → `customInstruments.saveToLibrary(patch.name, patch)`; "as .voxdaw file" → a minimal 1-track project (`serializeProject` + `downloadJSON`) whose one track uses a fresh `custom:` id with the def embedded — opens as a normal project.

### Custom instruments (`customInstruments.js`, `customInstrumentSynth.js`)
A generated patch promoted to a selectable **workstation** instrument. `makeCustomInstrumentNode(patch)` (customInstrumentSynth.js) is a composite that bakes the whole patch — **N per-layer subgraphs** `makePatchSynth(layer) → Tone.Filter → [makeFxGraph…] → layerVol` summed into `Tone.Volume(patch.volume)` (the same `buildLayerChain` `useAIInstrument` uses) — into one node that **quacks like a PolySynth**: trigger/`set`/`releaseAll`/`dispose`/`toSeconds` **fan to every layer synth** (`activeVoices` sums across layers), `.connect`/`.disconnect` delegate to the output Volume. Because `synthIsRinging` duck-types on `typeof s.activeVoices === 'number'` and `applyEnvelope` on `synth.set`, the engine treats it as a poly synth **with no changes** (region synth, audition, ringing/prune, envelope-sync, offline bounce all just work). **`toSeconds` is load-bearing** — `Tone.Part` callbacks call it on the scheduled instrument, and a plain class (unlike a real Tone node) doesn't inherit it. The track's own insert-FX rack sits *after* the instrument, untouched. **Glide is supported**: `makeMonoGlideVoice(patch)` builds a MONO stack of the composite (one bare voice per layer from `patchSynth.patchVoiceSpec` → filter → FX → level) exposing ONE `detune`/`volume` that `scheduleGlideSynthVoice` writes — a shared `Tone.Signal` detune connected into every layer voice (each voice's own `detune.value` holds its static octave+semitone offset; the scheduler's portamento curve adds on top) + one master `Tone.Volume` — so a multi-layer custom glide keeps every layer's tone AND octave. (Only **drum kits** disable the glide tool.)

`customInstruments.js` is the registry: an in-memory `Map` (id `custom:<uuid>` → `{id,name,patch}`) = **library** (persisted to `localStorage['voxdaw.customInstruments']`, listed in the RegionEditor instrument dropdown's "custom" optgroup) ∪ **project-registered** (`registerInstruments`, runtime-only, from an imported `.voxdaw`). `makeSynth`/`defaultEnvelopeFor` resolve custom ids through it; a missing id → graceful default synth. **`defaultEnvelopeFor(custom)`** returns the layer's own ADSR for a **single**-layer patch but **`null` for multi**-layer — otherwise the workstation's envelope-sync (effect 3c) applies one default through `CustomInstrumentNode.set`, clobbering every layer's own envelope; `customMaxRelease` supplies the tail estimate the now-null return can't. **Portability:** `serializeProject` embeds `customInstruments: [{id,name,patch}]` for the ids a project's tracks use (`customInstrumentsForIds`); `deserializeProject` returns them and `applyProjectData` calls `registerInstruments` **before** the audio reconciler builds, so someone else's import reconstructs the instrument. Additive field — **SCHEMA_VERSION intentionally NOT bumped** (old apps fall back to a default synth for unknown ids). Every patch re-runs `sanitizePatch` on register (the trust boundary). The performance keyboard spans octaves **−2..8** (`MIN_OCT = -2`, both surfaces).

**Library persistence:** a project-registered instrument plays but isn't permanent until saved to the library. `applyProjectData` prompts (`window.confirm`) on load when a project uses custom instruments not yet `isInLibrary`, saving them via `addToLibrary`. Otherwise the track's ContextMenu has an **Add Instrument** item (above Export, shown only for custom-instrument tracks, grayed once `isInLibrary`) → `addToLibrary(track.instrument)`.

### Sampled (CPU friendly) mode — `customSampleStore.js` / `customSampleRenderer.js` (Phase 157)
DAW-style freeze for custom instruments: the instrument tab's **engine** group (custom tracks only) renders **every chromatic note C1–C7 (73, midi 24–96)** of the patch offline and plays back a **real `Tone.Sampler`** instead of the N-layer live composite — one buffer source per note vs N layer synths + filters + FX (LFO effects run continuously while connected; that is the saving).

- **Renderer** (`customSampleRenderer.js`): sequential `Tone.Offline` per note — `makeCustomInstrumentNode(patch)` inside the callback (audioBounce's proven pattern; callback MUST stay synchronous — Offline swaps the global Tone context), `triggerAttack(note, 0, 1)` held `SAMPLE_SUSTAIN_SEC = 16` (~8 bars @ 120 — the held-note ceiling of the mode), duration + longest layer release + `fxTailSec` ring-out (exported from `useWorkstationAudio`), capped 45 s; trailing-silence trim via `trimExportBuffer({startSec: 0})` so fading (sustain-0) patches produce short samples — Phase 157's 20 s-decay fades and this mode are complementary. Abort granularity = one note (`AbortController`); progress via `onProgress(done, 73)`. Velocity rendered at 1.0 — playback velocity is a linear gain scalar in Sampler and PolySynth alike.
- **Store** (`customSampleStore.js`, deliberately Tone-free — decodes via a throwaway `OfflineAudioContext` at the WAV's own rate, zero resample): IndexedDB db `voxdaw` **v2** (projectStore owns the upgrade; new store `customSamples`, out-of-line keys). **One record per note** (`` `${id}|n${midi}` `` → `{wav: ArrayBuffer (16-bit PCM via encodeWavArrayBuffer), midi}`) + a **meta record written LAST** (`` `${id}|meta` ``) as the completeness marker — cancelled renders leave invisible partials, deletes are one key-range. In-memory cache (`getSampleSet` — SYNC, consumed by `makeSynth`) holds **native AudioBuffers** (context-free → valid in live AND offline/bounce Samplers); `getSampleStatus` (`unknown|loading|ready|missing`) + `subscribeSampleSets` drive the UI and the hook's `sampleSetsVersion` state. `saveSampleSet` rolls back partials on quota errors; `customInstruments.deleteCustom` fire-and-forgets `deleteSampleSet` (no ~200 MB orphans).
- **Engine integration:** `track.useSampled` (additive `.voxdaw` field, SCHEMA_VERSION NOT bumped; WAVs never enter the file — imports elsewhere fall back to the live composite + a "render needed" panel state). `makeSynth(id, {useSampled})` → cache hit builds `new Tone.Sampler({urls, release: set.release})`; being a genuine Sampler opts it into the `_activeSources` paths for free (ghost-note fix, silenceAll hard-cut, live+offline glide `scheduleGlideSamplerChain`, `synthIsRinging`) — **the private-API re-verify-on-Tone-bump rule now also covers sampled custom tracks**. Rebuild identity is **`synthKeyFor(track)`** = `instrument|S-or-L` ('S' only when flag on AND status 'ready'): the region reconciler compares it instead of bare instrument (rename `instrumentChanged→synthChanged`; same path disposes glide pools and rebuilds Parts, which close over the synth), with `sampleSetsVersion` in its deps so a finished prime hot-swaps live→Sampler; the audition cache keys on it too. `defaultEnvelopeFor(id, {useSampled})` → `{attack: 0, release: customMaxRelease}` (A/R-only chassis, `sampledMelodic` includes sampled customs); **toggling clears `track.envelope` in both directions** (instrument-kind change — `handleInstrumentChange` precedent, which now also clears `useSampled`). Sampler loading bookkeeping is intentionally NOT wired — buffer urls make `loaded` true synchronously.
- **Known compromises:** short notes fade with the patch's max release rather than replaying the exact synth release + FX ring (the standard sampled-instrument trade); held notes cap at ~16 s; a fully-sustaining 5-layer patch costs ~200 MB IDB / ~400 MB decoded (fading patches trim to a fraction — the UI copy steers there).
